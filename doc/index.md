---
layout: home
title: Alias Manager
hero:
  name: am
  text: Alias Manager
  tagline: Fast, safe CLI for managing shell aliases directly in your shell config.
  actions:
    - theme: brand
      text: Get Started
      link: /getting-started
    - theme: alt
      text: Command Reference
      link: /commands
features:
  - title: Works in Your Dotfiles
    details: Edits .zshrc, .bashrc, or config.fish directly while keeping comments and structure intact.
  - title: Safe by Default
    details: Creates .bak backups and uses atomic writes to protect your shell configuration.
  - title: Fast Single Binary
    details: Zero runtime dependencies, cross-platform, and ready to ship via Homebrew or source builds.
  - title: Immediate Changes
    details: The am init wrapper re-sources your shell after updates so aliases apply right away.
---

Welcome to the Alias Manager (am) docs. Use the navigation to install, learn the workflow, and ship safely.
